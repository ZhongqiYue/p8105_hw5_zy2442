...
<img src="p8105_hw5_zy2442_files/figure-gfm/unnamed-chunk-13-1.png" width="90%" />
  ...
  - As shown by the second plot, the sample average of μ̂ across tests
    ...
    of μ as the effect size (true mean values) increases. As shown by
    the previous test power plot, as the effect size increases, the
    proportion of times the null was rejected also increases. Therefore,
    when the effect size increases, the null hypothesis mu = 0 is more
    likely to be rejected, which means that the estimate mu values will
    depart more from zero. Thus, as the effect size increases, the
    estimated mu value for which the null is rejected will be more
    closer to its true mu value.
